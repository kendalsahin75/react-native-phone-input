...
export default class PhoneInput extends Component {
  ...
  onChangePhoneNumber(number) {
    ...
    const actionAfterSetState = onChangePhoneNumber
      ? () => {
        onChangePhoneNumber("phone", number);
      }
      : null;
  ...
  renderMiniIcon = () => {
    const { isMiniIcon, miniIconSource, miniIconStyle } = this.props
    const { disabled }  = this.state;
    if (isMiniIcon) {
      return (
        <TouchableWithoutFeedback
          onPress={this.onMiniPress}
          disabled={disabled}
        >
          <Image
  ...
  render() {
    ...
    return (
      <View style={[styles.container, this.props.containerStyle]}>
        ...
        {this.renderMiniIcon()}
        <View style={{ flex: 1, marginLeft: this.props.offset || 4 }}>
          <TextComponent
            ref={ref => {
              this.inputPhone = ref;
            }}
